...
class DictApiTest {
    ...
    void list_happy_admin_shouldSucceed() throws Exception {
        mockMvc.perform(get("/api/v1/dict-types").header("Authorization", "Bearer " + adminToken()))
    ...
    void list_bad_noToken_should401() throws Exception {
        mockMvc.perform(get("/api/v1/dict-types"))
                .andExpect(status().isUnauthorized());
    }

    @Test
    void list_bad_noperm_should403() throws Exception {
        mockMvc.perform(get("/api/v1/dict-types").header("Authorization", "Bearer " + nopermToken()))
    ...
    void create_happy_shouldSucceed() throws Exception {
        ...
        String body = objectMapper.writeValueAsString(Map.of("code", code, "name", "测试", "is_public", false, "status", 1));
        mockMvc.perform(post("/api/v1/dict-types").header("Authorization", "Bearer " + adminToken()).contentType(MediaType.APPLICATION_JSON).content(body))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(0));
    }
    ...
    void create_bad_duplicate_shouldFail() throws Exception {
        String body = objectMapper.writeValueAsString(Map.of("code", "gender", "name", "性别", "is_public", false, "status", 1));
        mockMvc.perform(post("/api/v1/dict-types").header("Authorization", "Bearer " + adminToken()).contentType(MediaType.APPLICATION_JSON).content(body))
    ...
    void getById_bad_notFound_should404() throws Exception {
        mockMvc.perform(get("/api/v1/dict-types/99999").header("Authorization", "Bearer " + adminToken()))
                .andExpect(status().is4xxClientError());
    }
}
