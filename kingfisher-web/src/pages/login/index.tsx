      ...
      label: (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 0' }}>
          <span>
            ...
            {a.username}
            {a.password ? <span style={{ color: '#52c41a', fontSize: 12, marginLeft: 8 }}>已存密码</span> : null}
          </span>
          <span
            style={{ color: '#ff4d4f', cursor: 'pointer', fontSize: 12, padding: '2px 6px' }}
            onMouseDown={(e) => { e.stopPropagation(); e.preventDefault(); handleRemoveAccount(a.username); }}
          >
            删除
          </span>
        </div>
        ...
        <Form form={form} onFinish={onFinish} size="large">
          ...
          <Form.Item style={{ marginBottom: 8 }}>
            <div style={{ display: 'flex', gap: 16 }}>
              <Checkbox checked={rememberAccount} onChange={(e) => { setRememberAccount(e.target.checked); if (!e.target.checked) setRememberPwd(false); }}>
                记住账户
              </Checkbox>
              <Checkbox
                checked={rememberPwd}
