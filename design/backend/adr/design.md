...
用户-角色关联方案：单角色（`users.role_id`）vs 多角色（`user_roles` 关联表）。

### 决策

v1 采用**单角色模型**。`users` 表一个 `role_id` 字段，JWT claims 为 `role string`。

### 原因

| | 单角色 | 多角色 |
|------|--------|--------|
| 复杂度 | 低，一个 JOIN | 中，需聚合多角色权限 |
| JWT | `role` 字符串 | 需 claims 存角色数组或仅存 user_id |
| 权限模型 | 够用（管理后台角色互斥） | 灵活但 80% 场景不需要 |
| 迁移成本 | 当前 | 后期可升（加表+改聚合+改 JWT） |

### 后果

- ✅ 实现简单，登录时一个 User.Preload("Role") 即完成
- ⚠️ 不支持"一个用户同时是 admin 和 editor"——此需求在管理后台罕见
- ⚠️ 若未来需要多角色：新增 `user_roles` 表、JWT claims 改 `roles []string`、权限聚合取并集
