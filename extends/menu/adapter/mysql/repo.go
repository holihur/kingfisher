package adapter
import ("context"; "gorm.io/gorm"; "kingfisher/extends/menu/domain")
...
func (r *MenuRepo) FindAll(ctx context.Context) ([]domain.Menu, error) {
    var pos []menuPO; err := r.db.WithContext(ctx).Order("sort ASC").Find(&pos).Error; if err != nil { return nil, err }
    menus := make([]domain.Menu, len(pos))
    for i, p := range pos { menus[i] = domain.Menu{ID: p.ID, ParentID: p.ParentID, Name: p.Name, Path: p.Path, Component: p.Component, Icon: p.Icon, Sort: p.Sort, Type: p.Type, Permission: p.Permission, Status: p.Status, CreatedAt: p.CreatedAt, UpdatedAt: p.UpdatedAt} }
    return menus, nil
}
func (r *MenuRepo) FindByID(ctx context.Context, id uint) (*domain.Menu, error) {
    var po menuPO; err := r.db.WithContext(ctx).First(&po, id).Error; if err != nil { return nil, err }
...
func (r *MenuRepo) FindByParentID(ctx context.Context, parentID uint) ([]domain.Menu, error) {
    var pos []menuPO; err := r.db.WithContext(ctx).Where("parent_id = ?", parentID).Order("sort ASC").Find(&pos).Error
    if err != nil { return nil, err }
    menus := make([]domain.Menu, len(pos))
    for i, p := range pos { menus[i] = domain.Menu{ID: p.ID, ParentID: p.ParentID, Name: p.Name, Path: p.Path, Component: p.Component, Icon: p.Icon, Sort: p.Sort, Type: p.Type, Permission: p.Permission, Status: p.Status} }
    return menus, nil
}
...
func (r *MenuRepo) HasChildren(ctx context.Context, parentID uint) (bool, error) {
    var count int64; err := r.db.WithContext(ctx).Model(&menuPO{}).Where("parent_id = ?", parentID).Count(&count).Error
    return count > 0, err
}
