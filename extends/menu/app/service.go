package app
import ("context"; "kingfisher/extends/menu/domain"; "kingfisher/extends/menu/adapter/mysql")
...
func (s *MenuService) GetTree(ctx context.Context) ([]domain.Menu, error) {
    menus, err := s.repo.FindAll(ctx); if err != nil { return nil, err }
    return buildTree(menus, 0), nil
}
func (s *MenuService) GetByID(ctx context.Context, id uint) (*domain.Menu, error) { return s.repo.FindByID(ctx, id) }
func (s *MenuService) Create(ctx context.Context, m *domain.Menu) error { return s.repo.Create(ctx, m) }
func (s *MenuService) Update(ctx context.Context, id uint, updates map[string]any) error { return s.repo.Update(ctx, id, updates) }
...
func buildTree(menus []domain.Menu, parentID uint) []domain.Menu {
    ...
    for _, m := range menus { if m.ParentID == parentID { m.Children = buildTree(menus, m.ID); result = append(result, m) } }
    return result
}
