package transport
import ("context"; "github.com/gin-gonic/gin"; "gorm.io/gorm"; "kingfisher/extends/menu/adapter/mysql"; "kingfisher/extends/menu/app")
type MenuModule struct{ handler *MenuHandler }
func NewMenuModule(db *gorm.DB) *MenuModule {
    repo := adapter.NewMenuRepo(db); svc := app.NewMenuService(repo)
...
func (m *MenuModule) RegisterProtected(r *gin.RouterGroup) {
    menus := r.Group("/menus"); menus.GET("/tree", m.handler.GetTree)
    menus.GET("/:id", m.handler.GetByID); menus.POST("", m.handler.Create)
    menus.PUT("/:id", m.handler.Update); menus.DELETE("/:id", m.handler.Delete)
}
