package adapter
import ("context"; "gorm.io/gorm"; "kingfisher/extends/config/domain")
...
func (r *ConfigRepo) GetAll(ctx context.Context) ([]domain.SystemConfig, error) {
    var pos []configPO; err := r.db.WithContext(ctx).Find(&pos).Error; if err != nil { return nil, err }
    configs := make([]domain.SystemConfig, len(pos))
    for i, p := range pos { configs[i] = domain.SystemConfig{ID: p.ID, Key: p.Key, Value: p.Value, Remark: p.Remark, CreatedAt: p.CreatedAt, UpdatedAt: p.UpdatedAt} }
    return configs, nil
}
func (r *ConfigRepo) GetByKey(ctx context.Context, key string) (*domain.SystemConfig, error) {
    var po configPO; err := r.db.WithContext(ctx).Where("key = ?", key).First(&po).Error
