package transport
import ("context"; "github.com/gin-gonic/gin"; "gorm.io/gorm"; "kingfisher/extends/config/adapter/mysql"; "kingfisher/extends/config/app")
type ConfigModule struct{ handler *ConfigHandler }
func NewConfigModule(db *gorm.DB) *ConfigModule {
    repo := adapter.NewConfigRepo(db); svc := app.NewConfigService(repo)
...
func (m *ConfigModule) RegisterProtected(r *gin.RouterGroup) {
    configs := r.Group("/configs"); configs.GET("", m.handler.GetAll)
    configs.GET("/:key", m.handler.Get); configs.PUT("/:key", m.handler.Set); configs.DELETE("/:key", m.handler.Delete)
}
