package transport
import ("context"; "github.com/gin-gonic/gin"; "gorm.io/gorm"; "kingfisher/extends/audit/adapter/mysql"; "kingfisher/extends/audit/app")
type AuditModule struct{ handler *AuditHandler; svc *app.AuditService }
func NewAuditModule(db *gorm.DB) *AuditModule {
    repo := adapter.NewAuditRepo(db); svc := app.NewAuditService(repo)
