...
func TestGenerateAndParse(t *testing.T) {
	...
	access, refresh, err := mgr.GenerateToken(context.Background(), 1, []uint{1}, []string{"admin"}, "admin", 1)
	if err != nil {
	...
	if len(claims.RoleIDs) != 1 || claims.RoleIDs[0] != 1 {
		t.Errorf("role_ids should be [1], got %v", claims.RoleIDs)
	}
	if len(claims.Roles) != 1 || claims.Roles[0] != "admin" {
		t.Errorf("roles should be [admin], got %v", claims.Roles)
	}
...
func TestExpiredToken(t *testing.T) {
	...
	_, _, err := mgr.GenerateToken(context.Background(), 1, []uint{1}, []string{"admin"}, "admin", 1)
...
func BenchmarkGenerateToken(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _, _ = mgr.GenerateToken(context.Background(), 1, []uint{1}, []string{"admin"}, "admin", 1)
	}
}
